...
import { fmt, isNativePlatform } from '../../utils/helper';
import { billService } from '../../services/bill.service';
import { cameraService } from '../../services/camera.service';

export const BillItemsEditor: React.FC = () => {
  const { items, addItem, updateItem, removeItem, participants, toggleItemParticipant, setBillTotal, setDiscountInfo } = useExpenseStore();
  ...
  const handleScan = async () => {
    try {
          ...
          expenses: [
            {
              quantity: 1,
              amount: 45000,
              name: "Bánh dẻo mỡ hành tóp mỡ trứng lòng đào",
            },
            { quantity: 1, amount: 27000, name: "Bánh tráng cuốn" },
          ],
          beforeDiscountTotal: 78000,
          actualTotal: 76000,
        };
      }

      console.log('response scanBill API', response);
      // Apply proportional discount only when beforeDiscountTotal > actualTotal
      // (guards against cases where beforeDiscountTotal is just the item subtotal
      //  while actualTotal already includes fees, which would inflate item prices)
      const discountRatio =
        response.beforeDiscountTotal > 0 && response.beforeDiscountTotal > response.actualTotal
      ...
      response.expenses.forEach(apiItem => {
        addItem({
          name: apiItem.name,
          price: (apiItem.amount / apiItem.quantity) * discountRatio,
      ...
      alert('Failed to scan and parse the receipt. Please try again or enter manually.');
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const handleAdd = () => {
    ...
    if (editingId) {
      updateItem(editingId, { name, price: Number(price), quantity: Number(qty) });
      setEditingId(null);
    } else {
  ...
  return (
    <div className="mb-6 bg-white p-5 rounded-3xl shadow-soft">
      ...
      <div className="flex flex-col gap-3 mb-4">
        {items.map((item) => (
          <div key={item.id} className="flex flex-col p-3 bg-surface-page rounded-2xl border border-[#ffd1dc]">
            <div className={`flex items-center justify-between transition-all ${editingId === item.id ? 'opacity-50' : ''}`}>
              ...
              <div className="text-right flex items-center gap-2">
                <p className="font-bold text-primary mr-2">{fmt(item.price * item.quantity)}</p>

                <button
                  onClick={() => handleEdit(item)}
                  className="w-8 h-8 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center hover:bg-blue-100 transition disabled:opacity-50"
                  disabled={!!editingId}
                >
            ...
            <div className="flex items-center gap-3 mt-3 pt-3 border-t border-[#ffd1dc] overflow-x-auto pb-3">
              <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider shrink-0" >Shared by:</span>
              <div className="flex items-center gap-4">
                {participants.map(p => {
                  const isShared = item.splits.some(s => s.userId === p.userId);
                  const splitAmt = item.splits.find(s => s.userId === p.userId)?.calculatedAmount || 0;
                  return (
                    <button
                      key={p.userId}
                    ...
                    >
                      <img src={p.avatarUrl} alt={p.name} className={`w-8 h-8 rounded-full ring-2 ring-white object-cover ${isShared ? 'border-[#ffd1dc]' : 'border-transparent'}`} />
                      {isShared && splitAmt > 0 && (
        ...
        {items.length === 0 && (
          <p className="text-sm text-text-muted text-center py-4 italic">No items added yet</p>
        )}
      </div>
      ...
      <div className="flex items-end gap-2 border-t border-[#ffd1dc] pt-4">
        <div className="flex-1">
          <label className="text-[10px] uppercase font-bold text-text-muted ml-2">Name</label>
          <input
            value={name}
        ...
        <div className="w-24">
          <label className="text-[10px] uppercase font-bold text-text-muted ml-2">Price</label>
          <input
            value={price}
        ...
        <div className="w-16">
          <label className="text-[10px] uppercase font-bold text-text-muted ml-2">Qty</label>
          <input
            value={qty}
        ...
        <button
          onClick={handleAdd}
          className={`${editingId ? 'bg-blue-500 text-white' : 'bg-[#FFE5EC] text-primary'} w-10 h-10 rounded-xl flex items-center justify-center transition shrink-0 shadow-sm active:scale-95`}
        >
